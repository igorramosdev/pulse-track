...
import { ReadersDashboard } from '@/components/stats/readers-dashboard'
...
export async function generateMetadata({ params }: ReadersPageProps): Promise<Metadata> {
  const { locale, token } = await params
  
  return {
    title: `Readers for ${token}`,
    description: `Page-level visitor analytics for ${token}`,
    robots: {
      index: false,
      follow: false,
    },
...
export default async function ReadersPage({ params }: ReadersPageProps) {
  const { locale, token } = await params
  ...
  return (
    <div className="py-12 sm:py-20">
      <div className="mx-auto max-w-5xl px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex flex-col items-center text-center sm:flex-row sm:items-start sm:text-left">
            ...
            <div className="flex-1">
              <h1 className="text-balance text-3xl font-bold tracking-tight sm:text-4xl">
                {tokenData.site_name ? `${tokenData.site_name} - Readers` : `Readers for ${token}`}
              </h1>
              {tokenData.site_url && (
        ...
        <ReadersDashboard token={token} dict={dict} />
      </div>
    </div>
  )
}
